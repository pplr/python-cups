...
file_path = os.path.join(os.path.dirname(__file__), "out.bin") 

def get_last_phy_payload():
    with open(file_path, 'rb') as file:
        phy_payload = file.read()
    return phy_payload
...
def main():
    with open(
        os.path.join(os.path.dirname(__file__), "full_response.bin"), 
mode="rb"
    ) as f:
        data = f.read()
    ...
    with connect(uri=response.tcUri + "/router-info", ssl_context=ssl_context) as websocket:
        websocket.send('{"router":"0008:00FF:FE4A:BC34"}')
    ...
    with connect(
        ...
        message = websocket.recv()
        print(message)
        phy_payload = get_last_phy_payload()
        print(f"phy_apyload: {phy_payload.hex()}")
